...
lazy val looty: Project = (project in file("looty"))
  ...
  .settings((fastOptJS in(Compile)) := (fastOptJS in(Compile)).dependsOn(WebKeys.assets in Assets).value)
  .settings((fullOptJS in(Compile)) := (fullOptJS in(Compile)).dependsOn(WebKeys.assets in Assets).value)
